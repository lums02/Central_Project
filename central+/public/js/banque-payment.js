            ...
            if (plan === 'decouverte' || priceMonthly === 0) {
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        ...
                        const registerUrl = `/register?type=banque_sang&plan=decouverte&period=trial&payment_method=gratuit&amount=0`;
...
function redirectToRegister(paymentMethod) {
    ...
    const registerUrl = `/register?type=banque_sang&plan=${selectedPlan}&period=${selectedPeriod}&payment_method=${paymentMethod}&amount=${amount}`;
    window.location.href = registerUrl;
}
